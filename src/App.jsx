...
function App() {
  ...
  const formatAIRecommendations = (text) => {
    ...
    return text
      // Convert markdown headers to styled headers (order matters - start with most hashtags)
      .replace(/^###### (.*$)/gim, '<h6 class="text-lg font-bold text-blue-400 mb-2 mt-4">$1</h6>')
      .replace(/^##### (.*$)/gim, '<h5 class="text-lg font-bold text-blue-400 mb-2 mt-4">$1</h5>')
      .replace(/^#### (.*$)/gim, '<h4 class="text-xl font-bold text-blue-300 mb-3 mt-5">$1</h4>')
